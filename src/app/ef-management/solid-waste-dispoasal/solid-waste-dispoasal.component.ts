...
import { OpenBurningOfWaste, ServiceProxy, SolidWasteDisposal } from 'shared/service-proxies/es-service-proxies';
import {DividerModule} from 'primeng/divider';
import { AppService } from 'shared/AppService';
import { Roles, UserActions } from 'shared/service-proxies/auth-service-proxies';

@Component({
  selector: 'app-solid-waste-dispoasal',
  templateUrl: './solid-waste-dispoasal.component.html',
  styleUrls: ['./solid-waste-dispoasal.component.css']
})
export class SolidWasteDispoasalComponent implements OnInit {
  public roles = Roles
  ...
  solidWasteDisposal: SolidWasteDisposal = new SolidWasteDisposal()
  solidWasteDisposalFacs:SolidWasteDisposal [];
  constructor(
  ...
  ngOnInit(): void {
    ...
    this.wdApproach = this.masterDataService.wdApproach;
    this.climateZone = this.masterDataService.climateZone;


    if (this.id == -1) {
      this.solidWasteDisposal = new SolidWasteDisposal()
    }
  ...
  console.log(event);
    ...
    this.serviceProxy
      .getManyBaseOpenBurningOfWasteControllerWasteIncineration(
        ...
        this.solidWasteDisposalFacs = res.data;
...
async saveWasteFac(wasteFacForm:NgForm){
  ...
  if (wasteFacForm.valid) {
    ...
    if (this.isNewEntry) {
      ...
      this.serviceProxy
        .createOneBaseSolidWasteDisposalControllerSolidWasteDisposal(this.solidWasteDisposal)
        .subscribe((res: any) => {
      ...
      console.log("hhhhh")
      this.serviceProxy.updateOneBaseSolidWasteDisposalControllerSolidWasteDisposal(this.solidWasteDisposal.id, this.solidWasteDisposal)
        .subscribe(
          (res) => {
            this.messageService.add({
              severity: 'success',
