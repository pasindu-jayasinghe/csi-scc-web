...
import { ServiceProxy, WasteIncineration } from 'shared/service-proxies/es-service-proxies';

@Component({
  selector: 'app-waste-incineration',
  templateUrl: './waste-incineration.component.html',
  styleUrls: ['./waste-incineration.component.css']
})
export class WasteIncinerationComponent implements OnInit {
  ...
  wasteIncFac: WasteIncineration = new WasteIncineration()
  wasteIncFacs:WasteIncineration [];
  constructor(
  ...
  ngOnInit(): void {
    ...
    this.onChangeGasType('getAll')
    this.onChangeWasteCat('getAll')


    if (this.id == -1) {
      this.wasteIncFac = new WasteIncineration()
    }
    ...
    if (this.id > 0) {
      ...
      if (this.editEntryId && this.editEntryId > 0) {
        ...
        this.serviceProxy.getOneBaseWasteIncinerationControllerWasteIncineration(
          ...
          this.wasteIncFac = res;
          // this.onChangeWasteCat(this.wasteIncFac.wasteCategory)
        });
      }
...
async saveWasteFac(wasteFacForm:NgForm){
  ...
  if (wasteFacForm.valid) {
    ...
    if (this.isNewEntry) {
      ...
      this.serviceProxy
        .createOneBaseWasteIncinerationControllerWasteIncineration(this.wasteIncFac)
        .subscribe((res: any) => {
      ...
      console.log("hhhhh")
      this.serviceProxy.updateOneBaseWasteIncinerationControllerWasteIncineration(this.wasteIncFac.id, this.wasteIncFac)
        .subscribe(
          (res) => {
            this.messageService.add({
              severity: 'success',
