...
@Component({
  selector: 'app-net-zero-business-travel-list',
  templateUrl: './net-zero-business-travel-list.component.html',
  styleUrls: ['./net-zero-business-travel-list.component.css']
})
export class NetZeroBusinessTravelListComponent extends EmissionListBaseComponent implements OnInit {

  rows: number = 10;
  loading: boolean;
  netZeroBusinessTravelData: any[];
  esCode: PuesDataReqDtoSourceName = PuesDataReqDtoSourceName.Net_zero_business_travel
  ...
  constructor(
    ...
    public netZeroBusinessTravelActivityDataControllerServiceProxy: NetZeroBusinessTravelActivityDataControllerServiceProxy,
  ...
  load(event: LazyLoadEvent) {
    ...
    if(this.listLoadable()){
      this.netZeroBusinessTravelActivityDataControllerServiceProxy
        .getAllBussinesTravelData(pageNumber,this.rows,
  ...
  edit(groupNumber: number) {
    this.router.navigate(['../net-zero-business-travel-edit'], {queryParams: { id: groupNumber }, relativeTo:this.activatedRoute });
  }

  view(groupNumber: number) {
    this.router.navigate(['../net-zero-business-travel-view'], { queryParams: { id: groupNumber } , relativeTo:this.activatedRoute });
  }

  new() {
    this.router.navigate(['../net-zero-business-travel-add'], {relativeTo:this.activatedRoute});
  }
  ...
  uploadExcell() {
    super.uploadExcell(PuesDataReqDtoSourceName.Net_zero_business_travel,this);
  }
  
  downloadExcel(){
    super.downloadExcel(PuesDataReqDtoSourceName.Net_zero_business_travel, this);
  }

  watchVideo(){
    super.watchVideo(PuesDataReqDtoSourceName.Net_zero_business_travel);
  }
  async deleteWholeGroup(groupnumber:string) {

    this.netZeroBusinessTravelActivityDataControllerServiceProxy.deleteWholeGroup(groupnumber)
      .subscribe(res => {
